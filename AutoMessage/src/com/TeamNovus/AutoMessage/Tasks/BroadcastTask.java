...
public class BroadcastTask implements Runnable {
	...
	@Override
	public void run() {
		if (MessageLists.getExactList(name) != null && AutoMessage.instance.getConfig().getBoolean("settings.enabled")) {
			MessageList list = MessageLists.getExactList(name);

			if (list.isEnabled() && list.hasMessages() && !(list.isExpired())) {
				if (Bukkit.getServer().getOnlinePlayers().size() >= AutoMessage.instance.getConfig().getInt("settings.min-players")) {
					...
					if (AutoMessage.instance.getConfig().getBoolean("settings.log-to-console")) {
